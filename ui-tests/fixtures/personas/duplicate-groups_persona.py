"""
Fixture persona: an ACP persona whose agent advertises two config options in
the same category (two `model`, two `mode`), to exercise the client's array-order
tie-breaking. See ../agents/duplicate_groups_agent.py.
...
_AGENT_SCRIPT = os.path.join(_AGENTS_DIR, "duplicate_groups_agent.py")
_AVATAR_PATH = os.path.join(
    os.path.dirname(jupyter_ai_acp_client.__file__), "static", "goose.svg"
)


class DuplicateGroupsTestPersona(BaseAcpPersona):
    """Test-only ACP persona advertising two options per category."""
    ...
    def defaults(self) -> PersonaDefaults:
        return PersonaDefaults(
            name="Duplicate Groups Agent",
            description="Test-only ACP persona with two options per category.",
