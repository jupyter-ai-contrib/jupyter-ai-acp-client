"""
Fixture persona for E2E tests: a second ACP persona backed by the same
`hello_agent.py`, but replying with a distinct word ("bonjour") so a test can
tell which persona answered. Used by persona-control.spec.ts to verify that
switching the picker routes each message to the selected persona.
...
class Hello2TestPersona(BaseAcpPersona):
    """Test-only ACP persona that always replies 'bonjour'."""

    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            executable=[sys.executable, _AGENT_SCRIPT, "--reply", "bonjour"],
            **kwargs,
        )
    ...
    def defaults(self) -> PersonaDefaults:
        return PersonaDefaults(
            name="Hello Agent Two",
            description="Test-only ACP persona that always replies 'bonjour'.",
