...
class JaiAcpClient(Client):
    ...
    async def _init_connection(self) -> ClientSideConnection:
        ...
        await conn.initialize(
            ...
            client_info=Implementation(
                name="Jupyter AI", title="Jupyter AI ACP Client", version="0.1.0"
            ),
        )
        return conn
        ...
        async with lock:
            ...
            try:
                ...
                response = await conn.prompt(
                    prompt=content_blocks, session_id=session_id
                )
                ...
                persona.log.info(
                    f"prompt_and_reply: completed for session {session_id}"
                )
                return response
            except Exception:
                persona.log.exception(
                    f"prompt_and_reply: failed for session {session_id}"
                )
                raise
            finally:
    ...
    def _handle_agent_message_chunk(
        self, session_id: str, update: AgentMessageChunk
    ) -> None:
        ...
        persona.log.info(
            f"agent_message_chunk: {len(text)} chars, tool_calls={len(serialized_tool_calls)}"
        )
    ...
    async def session_update(
        ...
        **kwargs: Any,
        ...
        if persona:
            persona.log.info(
                f"session_update: {type(update).__name__} for session {session_id}"
            )
    ...
    async def write_text_file(
        ...
        try:
            ...
            raise RequestError.internal_error(
                {"path": path, "error": f"Permission denied: {e}"}
            )
        except OSError as e:
