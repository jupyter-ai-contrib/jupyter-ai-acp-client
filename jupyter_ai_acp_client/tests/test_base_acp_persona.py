...
class TestSessionLegacyModelsHandoff:
    ...
    async def test_load_session_clears_stale_models_on_empty_pop(self):
        persona = _real_session_persona()
        # A stale payload from an earlier session must not leak across loads.
        persona._acp_legacy_models = {"currentModelId": "old"}
        client = MagicMock()
